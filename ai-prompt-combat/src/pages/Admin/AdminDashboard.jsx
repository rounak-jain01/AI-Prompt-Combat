...
import { 
  collection, doc, updateDoc, onSnapshot, deleteDoc 
...
import { 
  ...
  Link as LinkIcon, Copy, Check, AlertTriangle
} from "lucide-react";
...
export default function AdminDashboard() {
  ...
  const [globalSettings, setGlobalSettings] = useState({
    round1Open: false, round2Open: false, targetDate: "", updates: [],
  ...
  const [inviteLink, setInviteLink] = useState(""); // 🔗 Generated Link Storage
  ...
  const handleForceRefresh = () => { 
      toast.loading("Re-syncing Data...", { duration: 1000 }); 
      setupListeners(); 
  };

  // --- 2. USER MANAGEMENT ACTIONS ---

  // 🔥 CREATE USER (Backend API Call)
  const handleCreateUser = async () => {
      ...
      try {
          ...
          if (res.ok && data.success) {
              ...
              setInviteLink(data.inviteLink); // Save link to show in UI
          } else {
              throw new Error(data.detail || "Failed to create user");
          }
      } catch (e) {
          console.error(e);
          toast.error(e.message || "Server Error", { id: tId });
      }
  };

  // DELETE USER (Firestore Only)
  const handleDeleteUser = async (userId) => {
      if(!window.confirm("⚠️ DANGER: This will permanently delete the user from the database.")) return;
  ...
  const quickUpdateStatus = (round, status) => {
      setSelectedUser(prev => ({ ...prev, [`${round}_status`]: status }));
  };
  ...
  const handleDateChange = (e) => {
      ...
      updateGlobalSetting("targetDate", utcDate);
  };
    ...
    <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-[#D4AF37] selection:text-black relative">
      ...
      <header className="sticky top-0 z-40 bg-[#0A0A0A]/95 backdrop-blur-md border-b border-white/5 p-4 md:px-8 flex justify-between items-center shadow-lg">
        <div className="flex items-center gap-4">
            ...
            <div>
                <h1 className="text-xl font-bold tracking-tight text-white">ADMIN <span className="text-[#D4AF37]">NEXUS</span></h1>
                <p className="text-[10px] text-green-500 font-mono flex items-center gap-1 uppercase tracking-widest"><Wifi size={10}/> System Online</p>
            </div>
        </div>
        <div className="flex gap-3">
            <button onClick={handleForceRefresh} className={`p-2 rounded-lg border border-white/10 text-gray-400 hover:text-white ${isRefreshing ? "animate-spin text-[#D4AF37]" : ""}`} title="Force Refresh"><RefreshCw size={18} /></button>
            <button onClick={() => navigate("/lobby")} className="text-gray-400 hover:text-white text-sm flex items-center gap-2 px-4 py-2 bg-white/5 rounded-lg border border-transparent hover:border-white/10"><LogOut size={16} /> Exit</button>
        </div>
      </header>
      ...
      <div className="flex border-b border-white/5 bg-[#080808]">
          {['overview', 'users', 'lobby'].map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab)} className={`flex-1 py-4 text-sm font-bold uppercase tracking-widest transition-all ${activeTab === tab ? "text-[#D4AF37] border-b-2 border-[#D4AF37] bg-[#D4AF37]/5" : "text-gray-500 hover:text-white hover:bg-white/5"}`}>{tab}</button>
          ))}
      </div>
        ...
        {activeTab === "overview" && (
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className={`p-6 rounded-2xl border transition-all ${globalSettings.round1Open ? "bg-emerald-900/10 border-emerald-500/50" : "bg-[#111] border-white/10"}`}>
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="text-lg font-bold flex items-center gap-2 text-white"><Terminal size={20}/> Round 1</h3>
                            <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase ${globalSettings.round1Open ? "bg-emerald-500 text-black" : "bg-gray-800 text-gray-400"}`}>{globalSettings.round1Open ? "OPEN" : "CLOSED"}</span>
                        </div>
                        <button onClick={() => updateGlobalSetting("round1Open", !globalSettings.round1Open)} className={`w-full py-3 rounded-lg font-bold uppercase tracking-widest ${globalSettings.round1Open ? "bg-red-500 hover:bg-red-600 text-white" : "bg-emerald-500 hover:bg-emerald-600 text-black"}`}>
                    ...
                    <div className={`p-6 rounded-2xl border transition-all ${globalSettings.round2Open ? "bg-[#D4AF37]/10 border-[#D4AF37]" : "bg-[#111] border-white/10"}`}>
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="text-lg font-bold flex items-center gap-2 text-white"><FileVideo size={20}/> Round 2</h3>
                            <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase ${globalSettings.round2Open ? "bg-[#D4AF37] text-black" : "bg-gray-800 text-gray-400"}`}>{globalSettings.round2Open ? "OPEN" : "CLOSED"}</span>
                        </div>
                        <button onClick={() => updateGlobalSetting("round2Open", !globalSettings.round2Open)} className={`w-full py-3 rounded-lg font-bold uppercase tracking-widest ${globalSettings.round2Open ? "bg-red-500 hover:bg-red-600 text-white" : "bg-[#D4AF37] hover:bg-[#b8952b] text-black"}`}>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <StatBox label="Banned" value={users.filter(u => u.round1_status === 'disqualified' || u.round2_status === 'disqualified').length} color="text-red-500" />
                </div>
            </div>
        )}
            ...
            <div className="animate-in fade-in">
                ...
                <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-6 bg-[#111] p-4 rounded-xl border border-white/5">
                    <div className="relative w-full md:w-1/2">
                        ...
                        <input type="text" placeholder="Search..." className="w-full bg-black border border-white/10 rounded-lg py-2 pl-10 pr-4 text-white focus:border-[#D4AF37] outline-none"
                    ...
                    <div className="flex gap-2 overflow-x-auto w-full md:w-auto">
                        ...
                        <button onClick={() => { setIsAddModalOpen(true); setInviteLink(""); }} className="ml-2 bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded font-bold text-xs uppercase flex items-center gap-2 shadow-lg"><UserPlus size={16}/> Add</button>
                    </div>
                </div>
        ...
        {activeTab === "lobby" && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in fade-in">
                ...
                <div className="bg-[#111] border border-white/10 rounded-2xl p-6">
                    <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2"><Calendar size={20} className="text-[#D4AF37]" /> Timer</h3>
                ...
                <div className="bg-[#111] border border-white/10 rounded-2xl p-6">
                    ...
                    <div className="flex gap-2 mb-4">
                        <input type="text" placeholder="Announcement..." className="flex-1 bg-black border border-white/20 rounded p-3 text-white focus:border-blue-400 outline-none"
                            value={newMessage} onChange={(e) => setNewMessage(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleAddUpdate()} />
                        <button onClick={handleAddUpdate} className="bg-blue-600 text-white px-4 rounded hover:bg-blue-500 font-bold text-xs">SEND</button>
                    </div>
                    <div className="bg-black/50 rounded-xl p-4 h-48 overflow-y-auto border border-white/5 space-y-2 mb-2 custom-scrollbar">
                        {globalSettings.updates && globalSettings.updates.map((msg, i) => (
                            <div key={i} className="flex justify-between items-start text-sm border-b border-white/5 pb-2"><div><span className="text-gray-500 text-xs font-mono mr-2">[{msg.time}]</span><span className="text-gray-300">{msg.text}</span></div></div>
                        ))}
                    </div>
                    <button onClick={() => {if(window.confirm("Clear all?")) updateGlobalSetting("updates", [])}} className="text-xs text-red-500 hover:text-red-400 flex items-center gap-1"><Trash2 size={12} /> Clear</button>
                </div>
            </div>
        )}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-sm p-4">
            <div className="bg-[#111] border border-white/20 rounded-2xl w-full max-w-md shadow-2xl relative overflow-hidden animate-in zoom-in-95">
                <div className="p-6 border-b border-white/10 bg-[#151515] flex justify-between items-center">
                    <h2 className="text-xl font-bold text-white flex items-center gap-2"><UserPlus size={20} className="text-blue-400" /> Add User</h2>
                    ...
                    {!inviteLink ? (
                        <>
                            <div className="p-3 bg-blue-900/20 border border-blue-500/20 rounded text-xs text-blue-200 flex gap-2">
                                ...
                                <span>This will create a login account & generate a setup link.</span>
                            </div>
                            <div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-md p-4 overflow-y-auto">
            <div className="bg-[#111] border border-white/20 rounded-2xl w-full max-w-2xl shadow-2xl relative overflow-hidden animate-in zoom-in-95 my-10 flex flex-col">
                <div className="p-6 border-b border-white/10 flex justify-between items-center bg-[#151515]">
                    <div><h2 className="text-xl font-bold text-white flex items-center gap-2"><Edit3 size={20} className="text-[#D4AF37]" /> Manage User</h2><p className="text-xs text-gray-500 font-mono mt-1">{selectedUser.userId}</p></div>
                ...
                <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div><label className="text-[10px] text-gray-500 uppercase tracking-wider block mb-1">Name</label><input className="w-full bg-black border border-white/10 rounded p-2 text-white text-sm" value={selectedUser.fullName || ""} onChange={(e) => setSelectedUser({...selectedUser, fullName: e.target.value})} /></div>
                    ...
                    <div className="p-4 bg-emerald-900/10 border border-emerald-500/20 rounded-xl">
                        <div className="flex justify-between mb-3 items-center">
                            <h3 className="text-sm font-bold text-emerald-400 uppercase flex items-center gap-2"><Terminal size={16}/> R1 Image</h3>
                            <div className="flex gap-2">
                                <button onClick={() => quickUpdateStatus('round1', 'pending')} className="text-[10px] bg-gray-800 px-2 py-1 rounded hover:bg-white/10 text-white flex gap-1" title="Reset"><RotateCcw size={10}/> Reset</button>
                                <button onClick={() => quickUpdateStatus('round1', 'disqualified')} className="text-[10px] bg-red-900/50 text-red-400 px-2 py-1 rounded hover:bg-red-900 flex gap-1" title="Ban"><Ban size={10}/> Ban</button>
                            </div>
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="p-4 bg-amber-900/10 border border-[#D4AF37]/20 rounded-xl">
                        <div className="flex justify-between mb-3 items-center">
                            <h3 className="text-sm font-bold text-[#D4AF37] uppercase flex items-center gap-2"><FileVideo size={16}/> R2 Video</h3>
                            <div className="flex gap-2">
                                <button onClick={() => quickUpdateStatus('round2', 'pending')} className="text-[10px] bg-gray-800 px-2 py-1 rounded hover:bg-white/10 text-white flex gap-1" title="Reset"><RotateCcw size={10}/> Reset</button>
                                <button onClick={() => quickUpdateStatus('round2', 'disqualified')} className="text-[10px] bg-red-900/50 text-red-400 px-2 py-1 rounded hover:bg-red-900 flex gap-1" title="Ban"><Ban size={10}/> Ban</button>
                            </div>
                        </div>
                ...
                <div className="p-6 border-t border-white/10 bg-[#151515] flex justify-between items-center">
                    <button onClick={() => handleDeleteUser(selectedUser.userId)} className="text-red-500 hover:text-red-400 flex items-center gap-2 text-sm font-bold opacity-80 hover:opacity-100 hover:bg-red-500/10 px-3 py-2 rounded"><Trash2 size={16} /> Delete User</button>
                    <div className="flex gap-3">
                        ...
                        <button onClick={handleSaveUser} className="px-6 py-2 rounded bg-blue-600 text-white font-bold hover:bg-blue-500 flex items-center gap-2 shadow-lg"><Save size={16} /> Save</button>
                    </div>
                </div>
            </div>
        </div>
...
const StatBox = ({ label, value, color }) => (
    <div className={`p-4 rounded-xl border border-white/10 bg-[#111] flex flex-col items-center justify-center relative overflow-hidden group`}>
        ...
        <span className={`text-[10px] uppercase tracking-widest ${color} z-10 font-bold`}>{label}</span>
    </div>
);
